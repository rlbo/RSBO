...
python run_besd_REP.py besd_gw 0 0 gw10Two1
    - benchmark_name
        + besd_gw -- gw10One1, gw10Two1, gw10Two2, gw10Three1; gw20Three1, gw20Three2
        + besd_ky -- ky10One
        + besd_it -- it10
        + besd_pd -- pd10
        + besd_mc -- mcf2, mcf3
...
for kg_iteration in xrange(len_list_sampled_vals, problem.num_iterations):
    ...
    list_test_mu_ = []
    if problem.obj_func_min.getFuncName() in ['GridWorld', 'MountainCar']: # key_word = 'steps'
    ...
    elif problem.obj_func_min.getFuncName() in ['Puddle', 'GridWorld_Item']: # key_word = 'return'
        ...
        for pt_od, pt_x in enumerate(test_x):
            test_mu = -1.0 * kg_gp_cpp.compute_mean_of_points(learningcurve_pts[pt_od])
            test_mu = -1.0 * test_mu + problem.obj_func_min._meanval
            list_test_mu_.append(test_mu)
